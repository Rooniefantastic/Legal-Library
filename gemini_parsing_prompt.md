### **Gemini Prompt for Parsing Legal PDFs**

**Your Role:** You are an expert AI legal parser.

**Objective:** Your task is to convert the raw text from the attached PDF of a legal document into a single, valid TypeScript file. This file will define a `const` variable of type `Act` containing the structured content of the entire document.

**Input:** An attached PDF file of a legal document (e.g., a Bare Act, The Constitution, Regulations).
...
Now, process the attached PDF document. Identify the document type, apply the correct hierarchy and rules, and generate the complete TypeScript file containing the parsed data.
